...
// activated so the opening frame has something in view to react to (the
// affordance the "no instructions" spec line asks for), ship pointing "up"
// (the direction scroll and the world both advance in).
export function createInitialState(seed: RngState): GameState {
  ...
  const firstPlanet: Planet = {
    id: 1,
    position: { x: firstSpec.lane, y: firstSpec.scrollY },

...
| 2 | **Relative-speed landing makes late levels unlandable anyway**, because the ship must both match a fast drift and fight gravity. | R6 unit tests, then R8's `seekPilot` at level 3+ | Cap the drift component used by `isGentleLanding` at a constant, so scroll speed stops raising the landing bar past a point. One named constant. |
